...
- **Developer status injection**: when `dev_auth.is_developer(username)` is true (developer key proven), `build_developer_block()` (in `system_prompt.py`) injects a forceful "this user is a VERIFIED DEVELOPER" directive into the system prompt of Layers 2-5 ONLY (never Layer 1). Without it the model rationalizes the "key accepted" history message as a glitch and refuses to recognize the developer.
