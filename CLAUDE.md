...
- **All AI prompts are server-side only** (2026-08 change). The client no longer builds or sends system prompts: the old `buildContextMessages` / `buildToolsPrompt` / `buildCalendarPromptSection` / `buildContextExtrasSystemMessage` and the legacy worker-proxy streaming path were removed. Chat generation is **fully server-side and asynchronous**: `sendChatMessage()` POSTs `{message, prior_messages, grades, courses, assignments, posts, extras, grade_level}` to `POST /api/chats/<id>/messages`; the server appends the user message and runs the 5-layer pipeline in a **background job** (`_run_chat_job` in `schoology/server.py`, bounded by `AI_JOB_MAX_CONCURRENT`, default 4). The client polls `GET /api/chats/<id>/status` every 1.5s until the reply lands — so generation continues when the user switches chats/menus or closes the tab, and **multiple chats generate concurrently**. The legacy synchronous `POST /api/chat/layered` endpoint still exists for backward compatibility. Chat auto-titling (`POST /api/chats/auto-title`), soft-fail grade re-detection (`POST /api/grade/redetect`), and the agent password check (`POST /api/agent-verify`) all run server-side now.
- **Developer status injection**: when `dev_auth.is_developer(username)` is true (developer key proven), `build_developer_block()` (in `system_prompt.py`) injects a forceful "this user is a VERIFIED DEVELOPER" directive into the system prompt of Layers 2-5 ONLY (never Layer 1). Without it the model rationalizes the "key accepted" history message as a glitch and refuses to recognize the developer. The block ALSO states explicitly that developer status does NOT exempt the user from any content/safety policy (2026-08-19: without this, the model solved the developer's own homework test question — the dev block made it think "not a regular student" = graded-work policy doesn't apply).
- **Latency knobs** (2026-08-19, thinking was too slow): `MAX_REJECTION_ROUNDS = 3` (was 5); per-layer token budgets cut roughly in half (L2 400, L3 700, L4 short/medium/long 250/600/1000, L5 600); prior-message windows trimmed to last 6 messages at 400 chars (Layer 1: last 3 at 300). Keep these in mind before "making the model think harder".
...
- **Server endpoints** (all in `schoology/server.py`, all use `decode_auth_header()`):
  ...
  - `POST /api/chats/<id>/messages` — append user message + queue a background AI job (202; 409 if a job is already active for this chat)
  - `GET /api/chats/<id>/status` — poll endpoint: `{messages, pendingJob: {status: queued|running|failed|cancelled, error} | null, updatedAt}`. `pendingJob` is null once the turn is done
  - `POST /api/chats/<id>/cancel` — cooperative cancel (worker discards its result); `POST /api/chats/<id>/retry` — re-run a failed/cancelled job from its stored payload
  - `POST /api/chats/<id>/edit` — edit & resend: truncate at `{index}`, append `{content}`, queue a fresh job
  - `POST /api/chats/<id>/append` — append a synthetic message (tool history / soft notes) so client-generated messages persist
...
- **Frontend state** (in `schoology/index.html`):
  ...
  - `sendChatMessage()` POSTs `/api/chats` on first send to mint an id, then posts the message to `/api/chats/<id>/messages` and polls for the reply. The client no longer PUTs the messages array — **the server owns message persistence** (job replies, tool history via `/append`, and soft notes all land server-side). After the first assistant reply, `aiNameChat()` auto-titles the chat if the user hasn't renamed it -- via `POST /api/chats/auto-title` (DeepSeek call + title prompt are SERVER-SIDE).
  - Server-side replies store both `content` (raw, brackets intact — used for tool dispatch via `handleAICommands`) and `display` (bracket-stripped — used for rendering). `renderChatMessages()` prefers `display` when present.
  - Each active chat gets a poller (`startChatPolling`/`_chatPollTick`); pollers run in the background even when the chat isn't open, and `ensureChatPolling()` re-attaches one when a chat with a live job is (re)opened. Busy state is per-chat (`state._genBusy`); `state._aiBusy` mirrors the current chat's flag.
  - On process start, `_fail_stale_chat_jobs()` marks any `pendingJob` left in queued/running state as failed (their worker died with the old process); the client renders a Retry button that POSTs `/retry`.
  - Saves are debounced 500ms via `scheduleSaveCurrentChat()` in demo mode only (real mode is a no-op now).
